...
class TestSkiRunRecommender(unittest.TestCase):
    ...
    def test_dummy_features(self):
        ...
        self.assertTrue(
            all([col not in df_dummy.columns for col in self.recsys.DUMMY_FEATURES])
        )
    ...
    def test_transform_features(self):
        ...
        X_transform = self.recsys.transform_features(
            df=df_transform, features=list(df_transform.columns)
        )
    ...
    def test_trail_recommendations(self):
        ...
        """
        df_total = self.recsys.trail_recommendations(
            index=1000, n=5, resort="Alpine Meadows", color=None
        )
